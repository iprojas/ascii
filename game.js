...
function getFlagInfo(level, z, x, y){
  ...
  if (level.map_id === CUBE_FRAME) {
    if (Math.abs(x) < 100 && Math.abs(y) < 100) {
      ...
      if (seed == 0) {
        return {message: ["Nop", "Acá no pasa nada"],
              finished: false};
      }
      if (seed == 1) {
  ...
  if (level.map_id === FIGURE_EIGHT) {
    ...
    if (score >= 3) {
      return {message:["Felicitaciones, no ganaste nada!!", "( "+ score + " / 3)", finished:true};
    }

    return {message:["Vas a tener que seguir!", "(" + score + " / 3)"], finished:true};
  }
